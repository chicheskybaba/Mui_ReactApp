...
class MuiAddJob extends Component {     // use rce snippet to generate class base constructor element.  
	...
	submitHandler = e => {
		...
		axios
			.post('http://127.0.0.1:8000/api/jobs/', this.state)   // .post accepts two arguments - the urls that will receive post and the data (which is the state objects - 'this state')
			.then(response => {
				console.log(response)      // let us log response into the console if successful.
				this.setState({ successMsg: 'Job Successfully Added to the Database' })
			})
			.catch(error => {
				console.log(error)        // Also, we log error in the console if error.
				this.setState({ errorMsg: 'Job Not Correctly Added to the Database' })
			})
	}
		...
		return (                                     // The following are MUI-TextFields and MUI-Button for data input and submission. 
			<Stack marginBottom={5} textAlign='center' justifyItems={'left'}>
				<form onSubmit={this.submitHandler}>
					<Typography variant='h3' color={'secondary'}>Add New Job Vacancies</Typography>
